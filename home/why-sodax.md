...
| --- | --- |
| **Wallets** | Use **Swaps (Solver)** to offer cross-network swaps in your UI, plus **Bridge** primitives where needed for asset transfer. Use **Lend / Borrow (Money Market)** to integrate lending primitives. |
| **DEXs & Aggregators** | Use **Swaps (Solver)** to quote and execute cross-network intents and expand routing beyond single-network liquidity. |
...
| **Perp DEXs / Yield Apps** | Use **Swaps (Solver)** to accept deposits from other networks via swap-into-your-asset flows, then complete the deposit inside your app. Use **Lend / Borrow (Money Market)** to enable borrowed asset deposits with user collateral on other networks. |
